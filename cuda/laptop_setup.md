...
Context: On Ubuntu/Pop!_OS, NVIDIA drivers often "lazy load"—they only initialize when a monitor is connected or a graphical session requests them. In a headless or compute-only setup (where the dGPU handles CUDA but not the display), the OS ignores the card at boot, causing PyTorch to report CUDA not available or device not found.

The Fix: Add this to the root crontab (sudo crontab -e) to force the drivers to initialize immediately at boot.
